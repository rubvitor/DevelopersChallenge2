...
namespace FinantialManager.Infra.CrossCutting.Util
{
    public class OFXUtils
    {
        /// <summary>
        /// This method read OFX file without the header and convert to XML
        /// </summary>
        /// <param name="path">The path of file</param>
        /// <returns>XML File</returns>
        public static string ReadOFXtoXMLFromPath(string path)
        {
            try
            {
                if (!File.Exists(path))
        ...
        /// <param name="path">The path of file</param>
        /// <returns>XML File</returns>
        public static string ReadOFXtoXMLFromFile(IFormFile file)
        {
            try
            {
                if (file == null)
        ...
        public static string ReadOFXtoXMLFromArray(string[] lines)
        {
            try
            {
        ...
        private static string ProcessLinesOFX(string[] lines)
        {
            ...
            foreach (var line in lines)
            {
                if (line.StartsWith("<"))
                {
                    ...
                    if (!lineAdd.Contains("</"))
                    {
                        var idxFirstOFX = lineAdd.IndexOf(">") + 1;
                        var tag = lineAdd.Substring(0, idxFirstOFX).Replace("<", "</");
